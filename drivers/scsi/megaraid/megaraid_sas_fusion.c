	...
	for (i = 0; i < resetwaittime; i++) {
		...
		if (fw_state == MFI_STATE_FAULT) {
		 ...
		 if(instance->requestorId && reason) {
			 printk(KERN_WARNING "megasas: SR-IOV Found FW in FAULT"
...
int megasas_reset_fusion(struct Scsi_Host *shost, u8 reason)
{
	...
	if (abs_state != MFI_STATE_FAULT && instance->crash_dump_buf &&
		...
		printk("megasas: IO timeout is detected, forcibly FAULT Firmware,\n");
		...
		mutex_unlock(&instance->reset_mutex);
                do {
                        ...
                        printk("megasas: waiting for [%d] seconds for crash dump collection"
                ...
                if(atomic_read(&instance->adprecovery) ==
					MEGASAS_HBA_OPERATIONAL) {
                        printk("megasas: OCR done for IO timeout case\n");
                        return SUCCESS;
                } else {
                        printk("megasas: Controller is not operational after"
                                " 120 seconds wait for IO timeout case"
                                " in FW crash dump mode,\n");
                        printk("megasas: Ignore crash dump and do OCR/kill adapter\n");
...
transition_to_ready:
			...
			if (megasas_transition_to_ready(instance, 1)) {
				printk(KERN_WARNING "megaraid_sas: Failed to "
				       "transition controller to ready for "
				       "scsi%d.\n", instance->host->host_no);
				if(instance->requestorId && !reason )
