...
tg3_vmware_ioctl_cim(struct net_device *dev, struct ifreq *ifr)
{
	...
	case BRCM_VMWARE_CIM_CMD_ENABLE_NIC:
		DP(TG3_MSG_NETQ_VERBOSE, "enable NIC\n");

		rc = tg3_open(tp->dev);
		break;
	case BRCM_VMWARE_CIM_CMD_DISABLE_NIC:
		DP(TG3_MSG_NETQ_VERBOSE, "disable NIC\n");
	...
	case BRCM_VMWARE_CIM_CMD_REG_READ: {
		...
		case BRCM_VMWARE_REG_ACCESS_APE_REG:
			if (0x8ffc < rd_req->reg_offset) {
				DP_ERR("%s: "
				       "out of range: APE_REG "
				       "req reg: 0x%x\n",
				       "reg read", rd_req->reg_offset);
				rc = -EINVAL;
				break;
		...
		DP(TG3_MSG_NETQ_VERBOSE, "%s: reg: 0x%x value:0x%x",
			    "reg read", rd_req->reg_offset,
			    rd_req->reg_value);
		...
		case BRCM_VMWARE_REG_ACCESS_APE_REG:
			...
			tg3_ape_write32(tp, wr_req->reg_offset, wr_req->reg_value);
			break;
		default:
			DP_ERR("%s:invalid access method: access type: 0x%x\n",
		...
		DP(TG3_MSG_NETQ_VERBOSE, "get NIC param\n");
	...
	case BRCM_VMWARE_CIM_CMD_GET_NIC_STATUS:
		DP(TG3_MSG_NETQ_VERBOSE, "get NIC status\n");
...
static void tg3_netq_limit_dflt_queue_counts(struct tg3 *tp)
{
	 ...
	 */
	if (!tp->rxq_req) {
		/* Allocated 5 queues (4 + 1) by default. */
		tp->rxq_cnt = min_t(u32, tp->rxq_cnt + 1, 5);
	}

	if (!tp->txq_req) {
		tp->txq_cnt = min_t(u32, tp->txq_cnt, 4);
	}
}
...
#ifdef VX86_DEBUG
	printk(KERN_INFO "FW dump for Broadcom Tigon3 ethernet driver "
		   DRV_MODULE_NAME ".c:v" DRV_MODULE_VERSION "-BETA ("
		   DRV_MODULE_RELDATE ")\n");
#else
	printk(KERN_INFO "FW dump for Broadcom Tigon3 ethernet driver "
		   DRV_MODULE_NAME ".c:v" DRV_MODULE_VERSION "-release ("
		   DRV_MODULE_RELDATE ")\n");
#endif
