...
#endif
#ifndef _DEFINE_LL_RX_POLL /* BNX2X_UPSTREAM */
...
static _UP_UINT2INT bnx2x_calc_num_queues(struct bnx2x *bp)
{
	...
	nq = clamp(nq, (_UP_UINT2NONE)1, (_UP_UINT2NONE)BNX2X_MAX_QUEUES(bp));
	return nq;
}
			       ...
			       struct sk_buff *skb)
{
	...
	if (skb_shinfo(skb)->gso_size) {
		...
		/* In old distros it possible vlan-stripping would not be
		 * configured. If a packets with vlan would be received but
		 * no vlan device will be configured for that vlan, it
		 * might reach this default - thus we can't have this
		 * default print there.
		 */
		default:
...
_UP_STATIC int bnx2x_rx_int(struct bnx2x_fastpath *fp, int budget)
{
	...
	while (BNX2X_IS_CQE_COMPLETED(cqe_fp)) {
		...
		if ((bp->dev->mtu > ETH_MAX_PACKET_SIZE) &&
		    (len <= RX_COPY_THRESH)) {
#ifdef BCM_HAS_BUILD_SKB /* BNX2X_UPSTREAM */
#ifdef _HAS_NAPI_ALLOC_SKB /* BNX2X_UPSTREAM */
...
void bnx2x_set_num_queues(struct bnx2x *bp)
{
	...
	/* RSS for default queue */	
	if(bp->rss_on_default_queue) {
       		bp->num_ethernet_queues +=  bp->num_queues_on_default_queue;
   	}
#endif
...
static void bnx2x_bz_fp(struct bnx2x *bp, int index)
{
	 ...
	 */
#ifndef __VMKLNX__ /* BNX2X_UPSTREAM */
#ifdef NETIF_F_LRO /* BNX2X_UPSTREAM */
	 ...
	 */
	if (IS_FCOE_FP(fp) || bp->disable_tpa)
#else  /* ! BNX2X_UPSTREAM */
	/* We don't want TPA on FCoE, FWD and OOO L2 rings */
	if (IS_FCOE_FP(fp) || IS_OOO_FP(fp) || IS_FWD_FP(fp) || bp->disable_tpa)
#endif
		fp->mode = TPA_MODE_DISABLED;

#ifdef BNX2X_LEGACY_RX_CSUM /* ! BNX2X_UPSTREAM */
	if (bp->flags & LEGACY_DISABLE_TPA_FLAG)
		fp->mode = TPA_MODE_DISABLED;
#endif
}
...
#if (VMWARE_ESX_DDK_VERSION >= 55000) /* ! BNX2X_UPSTREAM */
   if(bp->rss_on_default_queue){
       BNX2X_DEV_INFO("Setting RSS on default queue with %d queues enabled\n",bp->num_queues_on_default_queue);
       bnx2x_netq_init_default_rss(bp);
   }
#endif
...
/* in OLD_VLAN vf vlgrp is always null */
 ...
 place_vlan_tagging:
	...
	bnx2x_esx_update_stats(bp, skb, xmit_type, pkt_size);
 ...
 */
	...
	fp->esx.rss_engine_id = BP_FUNC(bp);
#if (VMWARE_ESX_DDK_VERSION >= 55000) /* ! BNX2X_UPSTREAM */
	if(bp->rss_on_default_queue) {
		if(index == 0)
	               fp->esx.rss_engine_id = BP_FUNC(bp) + E2_FUNC_MAX;

       		if(bp->esx.rss_p_size)
               		rss_queue_index = BNX2X_NUM_RX_NETQUEUES(bp) + bp->esx.rss_p_size - 1;
          	else
               		rss_queue_index = BNX2X_NUM_RX_NETQUEUES(bp);

       		if(bp->count_rss_on_default < bp->num_queues_on_default_queue) {
           		if(index > rss_queue_index) {
               			DP(BNX2X_MSG_NETQ,"Allocating rss queue[%d] on default queue.\n",index);
               			alloc_skb = 1;
               			bp->count_rss_on_default++;
				fp->esx.rss_engine_id = BP_FUNC(bp) + E2_FUNC_MAX;
           		}
       		}
   	}
...
alloc_mem_err:
	...
	if ((IS_OOO_IDX(index) && ring_size < MIN_RX_AVAIL_OOO) ||
	    (!IS_OOO_IDX(index) &&
		ring_size < (fp->mode == TPA_MODE_DISABLED ?
...
void bnx2x_tx_timeout(struct net_device *dev)
{
	...
	bnx2x_esx_trigger_grcdump(bp, ESX_CXT_TXTO_TRIGGER);
