...
extern _UP_UINT2INT bnx2x_num_queues;
...
static inline int bnx2x_func_start(struct bnx2x *bp)
{
	...
	if (bp->geneve_dst_port) {
		/* Enable Geneve RSS */
		start_params->geneve_dst_port = bp->geneve_dst_port;
...
#if (VMWARE_ESX_DDK_VERSION >= 55000) /* ! BNX2X_UPSTREAM */
	 /* RSS configuration object for default queue*/
	if(bp->rss_on_default_queue)
       		bnx2x_init_rss_config_obj(bp, &bp->default_rss_conf_obj, bp->fp->cl_id,
                  bp->fp->cid, BP_FUNC(bp), BP_FUNC(bp) + E2_FUNC_MAX,
                  bnx2x_sp(bp, defq_rss_rdata),
                  bnx2x_sp_mapping(bp, defq_rss_rdata),
