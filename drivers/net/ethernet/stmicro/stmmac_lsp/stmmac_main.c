...
int stmmac_fixup_eee_disable(struct stmmac_priv *priv)
{
	...
	if (!phy_init_eee(priv->phydev, 1)) {
		ret = phy_ethtool_get_eee(priv->phydev, &edata);
		if (ret < 0)
			return ret;
...
static int stmmac_init_ptp(struct stmmac_priv *priv)
{
	...
	/* ptp clock could be already registered if
	 * we are coming out of suspend state
	 */
	if (priv->ptp_clock)
		return 0;
...
static int stmmac_init_phy(struct net_device *dev)
{
	...
	pr_debug("stmmac_init_phy: %s: attached to PHY %s UID 0x%x Link = %d\n",
		 dev->name, phydev->drv->name, phydev->phy_id, phydev->link);
...
static int stmmac_hw_setup(struct net_device *dev)
{
	...
	/* To enable/disable the IPC and do a further check for HW where
	 * this bit cannot be forced.
	 */
	ret = priv->hw->mac->rx_ipc(priv->ioaddr, priv->rx_csum);
	if (!ret) {
		pr_warn(" RX IPC Checksum Offload disabled\n");
		/* Reset both rx_coe and rx_csum */
...
static int stmmac_rx(struct stmmac_priv *priv, int limit)
{
	...
	while (count < limit) {
		...
		if (unlikely(status == discard_frame)) {
			...
			if (IS_ENABLED(CONFIG_STMMAC_VLAN_TAG_USED))
			       ...
			       netdev_features_t features)
{
	 ...
	 */
	priv->hw->mac->rx_ipc(priv->ioaddr, priv->rx_csum);
...
int stmmac_resume(struct net_device *ndev)
{
	...
	if (device_may_wakeup(priv->device)) {
		...
		/* reset the phy sothat its ready */
		if (priv->mii)
