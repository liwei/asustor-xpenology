...
static int rtl8366_reset(struct rtl8366_smi *smi)
{
	if (smi->hw_reset) {
		...
		return 0;
	} else if (gpio_is_valid(smi->gpio_reset)) {
...
static int __rtl8366_smi_init(struct rtl8366_smi *smi, const char *name)
{
	...
	/* Check if the reset via GPIO is valid (not mandatory parameter)
	 * and in case of it's ok go ahead to reset the Switch
	 */
	if (gpio_is_valid(smi->gpio_reset)) {
		err = gpio_request(smi->gpio_reset, name);
		if (err) {
			dev_err(smi->parent, "gpio_request failed %u err=%d\n",
				    smi->gpio_reset, err);
			goto err_free_sck;
		}
...
int rtl8366_smi_init(struct rtl8366_smi *smi)
{
	...
	/* Some HW do not provide the detection helper. */
	if (smi->ops->detect) {
...
int rtl8366_smi_probe_of(struct platform_device *pdev, struct rtl8366_smi *smi)
{
	...
	int reset = of_get_named_gpio(pdev->dev.of_node, "gpio-reset", 0);

	if (!gpio_is_valid(sck) || !gpio_is_valid(sda)) {
		dev_err(&pdev->dev, "gpios missing in devicetree\n");
		return -EINVAL;
	}

	smi->gpio_sda = sda;
	smi->gpio_sck = sck;
	smi->gpio_reset = reset;
