...
/*#define DEBUG 1*/
#ifdef DEBUG
#define DPRINTF(format, ...) pr_info("%s: " format, __func__, ##__VA_ARGS__)
#else
	...
	case SWITCH_TYPE_PORTS:
		...
		if (info->attrs[SWITCH_ATTR_OP_VALUE_PORTS]) {
			err = swconfig_parse_ports(skb,
				info->attrs[SWITCH_ATTR_OP_VALUE_PORTS], &val,
								dev->ports);
			if (err < 0)
				goto error;
		} else {
...
static int
swconfig_send_switch(struct sk_buff *msg, u32 portid, u32 seq, int flags,
		const struct switch_dev *dev)
{
	void *hdr;

	hdr = genlmsg_put(msg, portid, seq, &switch_fam, flags,
			SWITCH_CMD_NEW_ATTR);
	if (IS_ERR(hdr))
		return -1;
