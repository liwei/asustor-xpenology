...
static struct of_device_id st_mali_match[] = {
	{
	.compatible = "st,mali",
	},
	{},
...
MODULE_DEVICE_TABLE(of, st_mali_match);
...
static struct platform_driver mali_platform_driver = {
	...
	.driver =
	{
		...
		.pm = &mali_dev_pm_ops,
#endif
		.of_match_table = of_match_ptr(st_mali_match),
	},
};
...
static int st_mali_remove(struct platform_device *pdev)
{
...
static int st_mali_probe(struct platform_device *pdev)
{
	struct mali_gpu_device_data mali_gpu_data = {};
	struct device_node *np = pdev->dev.of_node;
	unsigned int size, rate;

	gpu_clk = devm_clk_get(&pdev->dev, "gpu-clk");
	if (IS_ERR(gpu_clk)) {
	 ...
	 * Use a default 224MiB in case no data is incoming from
	...
	if (of_property_read_u32(np, "mali-mem-size", &size) == 0)
		...
		mali_gpu_data.shared_mem_size = 0x0E000000;

	if (platform_device_add_data(pdev,
		&mali_gpu_data, sizeof(mali_gpu_data))) {
		MALI_PRINT_ERROR(("ST Mali memory registration failed.\n"));
		return -EEXIST;
	}
	...
	*/
	...
	pm_runtime_set_autosuspend_delay(&pdev->dev, 1000); /* msec */
	pm_runtime_use_autosuspend(&pdev->dev);
	pm_runtime_set_active(&pdev->dev);
	pm_runtime_enable(&pdev->dev);
	if (pm_runtime_enabled(&pdev->dev))
		MALI_DEBUG_PRINT(2, ("Mali registered in power domain\n"));
