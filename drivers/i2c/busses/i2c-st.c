...
static void st_i2c_hw_config(struct st_i2c_dev *i2c_dev)
{
	...
	val = i2c_dev->sda_min_width_us * (rate / 1000);
	val /= (prsc * 10000);
			...
			struct i2c_msg msgs[], int num)
{
	...
	/* Disable Tx and Rx not to be affected by glitches on the bus */
	st_i2c_clr_bits(i2c_dev->base + SSC_CTL, SSC_CTL_EN);
...
static int st_i2c_probe(struct platform_device *pdev)
{
	...
	snprintf(adap->name, sizeof(adap->name), "ST I2C(0x%x)", id);
	adap->nr = id;
