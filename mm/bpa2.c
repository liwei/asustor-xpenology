...
enum logical_memory_position {
	INSIDE_LOGICAL_MEMORY,
	OUTSIDE_LOGICAL_MEMORY,
	CROSSING_LOGICAL_MEMORY
};

static enum logical_memory_position
is_mem_low_or_high(unsigned long start,
		unsigned long size)
{
	unsigned long start_pfn, end_pfn;
#ifdef CONFIG_HAVE_MEMBLOCK
	struct memblock_region *reg;
	unsigned long prev_reg_end_pfn = 0;
#endif
...
#ifdef CONFIG_HAVE_MEMBLOCK
	/* memblock.memory.regions[] is ordered:
	 * first region is the lowest low_memory */
	for_each_memblock(memory, reg) {
		if ((start_pfn >= memblock_region_memory_base_pfn(reg)) &&
		    (end_pfn <= memblock_region_memory_end_pfn(reg)))
			return INSIDE_LOGICAL_MEMORY;
		if ((start_pfn >= prev_reg_end_pfn) &&
		    (end_pfn <= memblock_region_memory_base_pfn(reg)))
			return OUTSIDE_LOGICAL_MEMORY;
		prev_reg_end_pfn = memblock_region_memory_end_pfn(reg);
	}
	/* check if requested area is after last memblock.memory.regions[] */
	if (start_pfn >= prev_reg_end_pfn)
		return OUTSIDE_LOGICAL_MEMORY;
#else
	if ((start_pfn >= PAGECOUNT_TO_PFN(min_low_pfn)) &&
...
{
	int result;
	enum logical_memory_position pos;
	...
	if (start == 0) {
		...
		pos = INSIDE_LOGICAL_MEMORY;
	} else {
		pos = is_mem_low_or_high(start, size);
		if (pos == INSIDE_LOGICAL_MEMORY) {
			result = bpa2_reserve_low(part, start, size);
		} else if (pos == OUTSIDE_LOGICAL_MEMORY) {
			result = bpa2_init_high(part, start, size);
		} else {
			pr_err("bpa2: partition spans logical memory boundary\n");
			result = -EFAULT;
		}
	}
	if (result != 0) {
		pr_err("bpa2: failed to create '%s' partition\n", *names);
		goto fail;
	}
	...
	if (result != 0) {
		pr_err("bpa2: could not reserve '%s' partition resource\n",
				*names);
		goto fail;
	}
	...
	pr_info("bpa2: partition '%s' created at 0x%08lx (%s), size %ld kB (0x%08lx B)\n",
			*names, start,
			(pos == INSIDE_LOGICAL_MEMORY) ? "low" : "high",
			size / 1024, size);
		...
		int priority, const char *trace_file, int trace_line)
{
	...
	used_range->stack_trace.nr_entries = 0;
	used_range->stack_trace.max_entries = ARRAY_SIZE(used_range->stack);
	used_range->stack_trace.entries = used_range->stack;
	used_range->stack_trace.skip = 0;
	save_stack_trace(&used_range->stack_trace);
#endif
 ...
 */
unsigned long bpa2_free_pages(struct bpa2_part *part, unsigned long base)
{
...
#ifdef CONFIG_BPA2_ALLOC_TRACE

static void seq_print_stack_trace(struct seq_file *s, struct stack_trace *trace)
{
	void *addr;
	int i;
	...
	for (i = 0; i < trace->nr_entries; i++) {
		addr = (void *)trace->entries[i];
		seq_printf(s, "     [<%p>] %pS\n", addr, addr);
	}
}

#endif /* CONFIG_BPA2_ALLOC_TRACE */
		...
		for (range = part->used_list; range != NULL;
			...
			if (range->trace_file)
				seq_printf(s, " (%s:%d)", range->trace_file,
						range->trace_line);
			seq_puts(s, "\n");
			seq_print_stack_trace(s, &range->stack_trace);
#endif
			seq_printf(s, "\n");
		}
	}
